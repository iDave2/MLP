...
class Index {
  ...
  constructor(path, begin = 0, count = null) {
    ...
    if (count === null) // This is NOT WORKING from XHR...
      count = availElements
    // console.error(`  now count is ${count}`)
    assert(0 < count && count <= availElements,
      `count out of bounds; try something in [1, ${availElements}]`)
    this.count = count
   ...
   *  *element* as described in {@link Index}.  (Links don't appear
