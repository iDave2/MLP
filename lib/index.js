 ...
 *
 *  Is it helpful to rename files defining one class, like Index,
 *  with matching upper camel case file names?  Or shall we honor
 *  the unix tradition of two-finger typing?
 */
'use strict'
...
class Index {
  ...
  constructor(path) {
    ...
    // this.dataType could also serve as new View interface...
  ...
  /******************************************************************
   *  Method returns a Node.js `stream.Readable` over the window
   *  or slice of this index given by element indices
   *  `[begin, begin + count)`.
   *
   *  Both input parameters have defaults:  `begin=0` will start
   *  reading from beginning of Index and `count=null` causes all
   *  elements from `begin` to end of database to be included.  So
   *  if you just say `getReader()`, your reader will read the
   *  entire Index.
   *
  ...
  getReader(begin = 0, count = null) {
    ...
    // Validate requested element window.
